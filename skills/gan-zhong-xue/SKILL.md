...
先看**上次有没有没挖完的**，再读地图：
```bash
mkdir -p ~/.gan-zhong-xue
cat ~/.gan-zhong-xue/pending.md 2>/dev/null && echo "↑ 上次没挖完，先问用户要不要接着挖"
cat ~/.gan-zhong-xue/map.md 2>/dev/null || echo "（地图空白，第一次，本轮写第一格）"
test -f ~/.gan-zhong-xue/map.jsonl && tail -20 ~/.gan-zhong-xue/map.jsonl || true
```
- **pending 非空**：先告诉用户"上次你说想挖 N 个、只挖了 M 个，还剩这些：…接着挖吗？"
  要 → 优先挖 pending；不要 → 保留或清掉，进新的。**绝不让想挖的点默默消失。**
...
4. **写入本地地图（脱敏！禁止具体值，见隐私铁则）**。两份：
   - `~/.gan-zhong-xue/map.md`：人读的，分层清单 + 每格证据原话（**抽象化**）。
   - `~/.gan-zhong-xue/map.jsonl`：程序读的，供第 -1 步复验调度。每格一行：
     ```json
     {"概念":"某类权限能力的底层机制","改动来源":"<repo>@<commit>","分层":"🟡",
      "来源":"🟢记/用户自报","证据":"用户原话（脱敏）","用户更懂":"用户补的、代码里读不到的意图",
      "首次":"2026-07-06","上次复验":"2026-07-06","状态":"待复验",
      "备注":"知道丢弃了权限，不懂 capability 底层"}
     ```
   **每格必带时间戳 + "待复验"状态**——🟢只能标"今天理解"，不是"真懂"。真懂靠第 -1 步在未来
   命中时复验通过才改成"已复验/实"。**这一步不是记录，是给未来的自己埋一个可被证伪的赌注。**

### 第 4 步：中断即存档（真实工作随时打断学习，是常态不是 bug）
学习和任务会互相打断（干到一半想学 / 学到一半想起要干活，两个方向都会）。任一方向被打断：
- 立刻把**还没挖的点 + 当前挖到哪**写进 `~/.gan-zhong-xue/pending.md`（**脱敏，只记抽象概念**）。
