...
**修 issue #2"越问不懂的点愈多"——补全学习边界（Learning Boundary）。**

- **痛点（issue #2，真实问题来源）** —— 完全不懂 C++ 的用户挖一个改动，前置递归展开：架构 → 模板 →
  基本语法 → 指针 → ……越问越多、陌生名词越滚越多。**我们把这个问题解释为**：skill 缺少明确的
  学习边界——A 类前置缺口只有一路（插进图挖），没区分"深入会改变对当前靶解释的前置"和"不会改变的
  外部前置"，后者挖进图等于递归展开整门语言（**注：这是设计者对 issue #2 的解释，尚待真实 episode
  验证，不是实验结论**）。
- **修法：A 类岔路加第二判，前置分两路** —— 主判 = **两步测试，缺一不可**：① **最小语义测试**
  （只掌握这个概念的浅层语义一句话，能不能继续解释当前靶的行为/设计？）② **边际改变测试**（继续深入它，
  还会改变对**当前靶这几行代码**的解释吗？——按严判，防"深入 TLS/TCP 都算改变解释"的万能化）。
  **①能 且 ②不会 → 外部前置；①不能 或 ②会 → 靶内前置**。（"是不是通用语言基础"只是快速启发、
  不是判据——若本次 commit 本身是关于 `delete p`/所有权转移的，pointer 就是主线核心，是基础也不放行）
  - **靶内前置** → 进图挖（原路保留，含插一挖一 + 连锁封顶）；
  - **外部前置** → **搭桥放行**：一句话搭出当前级够用的最小理解，**主线立刻继续**；map 追加 ⏸外部前置
    格；指路但不强制（教材/课程系统都行，现在或以后随用户）。
  - **关键设计：搭桥式而非暂停式**。不是"停下来去上课、学完回来再继续"——那是把心流断在墙上；
    是铁律5 的"接住"：托过去，主线不断。外部补课是异步的、可选的。
- **⏸外部前置格（map 新增状态，必填"关联靶（靶ID）"+"关联节点"两字段）**：一条**关系记录**（概念×靶），
  不是概念状态——同一概念可对不同靶产生多条 ⏸，同概念同靶再撞上也可追加，历史只增不改。**调度检索顺序：
  先按当前靶筛"靶 ID"一致的 ⏸ 记录、再处理概念，绝不由概念反查 ⏸**（append-only 下同概念多条记录指向
  不同主线，由概念反查 = 把关系记录当回概念属性）。**持久靶 ID（让"字面对应"机器可执行）**：第0步定靶时
  生成 3-5 词 kebab-case 短标识（如 `dma-buffer-ownership`，抽象脱敏）——叫"持久"不叫"稳定"：可靠性
  只来自"一旦创建、后续复用"，不来自"同输入必出同 ID"。**复用只有两条路：用户明确指认（列已有靶 ID 让
  用户选）、或当前会话明确继承上一轮靶；否则生成新 ID，绝不因"语义相近"自动复用**（那是 ≈ 换马甲）。
  **关联节点引用议程图节点 ID/短标题，但它是定位锚不是"查表即恢复"**——当轮议程图不持久化，跨 session
  恢复仍需依据当前源码与议程重建上下文。**搭桥边界**：桥只搭一般概念语义；涉及当前代码具体行为/所有权/
  生命周期/控制流的事实，仍受铁律10 约束（先读源码、带路径行号）。无验证题、
  **永不复验内容本身**；靶 ID 命中 → 回"关联节点"的当初卡点继续，**原卡点是默认复验点、以"能否支撑
  当前靶"为限，不是充分复验**（验证形式=主线那级的预测题，考靶不考课）。用户**明确换靶**（"我要学 X"）
  → 忽略 ⏸、按全新主靶开梯子；**单纯问"X 是什么？"不构成换靶**（仍按 A 类前置走第二判，否则 issue #2
  复发）。防的滑坡：考外部概念→变课程系统；⏸拦换靶/概念反查→关系标签变概念属性。
- **外部前置是关系标签、不是概念属性（数据结构与检索都实现了）**：pointer 对"理解这次改动"是外部
  前置，对用户明确换靶"我要学 pointer"就是主靶——标签随靶失效；靠"关联靶/关联节点"字段 + 先靶后
  概念的调度落地，不停留在文字原则。
- **学习边界 = 二维**：纵向"别钻到晶体管"（v1.0.4 老原则，管深度）+ 横向"别追到整个语言体系"
  （v1.0.7 补，管广度）。靶外知识无论多基础，只给当前级所需的最小桥接、不展开其自身知识树。
- **examples/real-round-1.md 模板升级**：加"岔路记录"字段（前置概念清单、两步测试怎么判的、何时失控、
  外部补课回来主线还卡不卡）。**issue #2 是本次设计的真实问题来源；该机制尚待真实 episode 验证——
  实录未填入前模板保持占位，绝不伪造实证案例。**
- **自检 +1 条**（外部前置搭桥放行，含靶ID/两步测试四失败模式）。**机制变化约 62 行**（不含版本标注/header）——v1.0.7 的边界就是不借机建大：不加
  Learning Episode schema、不做课程集成、不动验证框架；下一个版本该做的是用 3-5 个真实 episode
  验证"搭桥放行是否既止住发散、又不降理解深度"——重点盯五个对抗场景：真外部前置该放行、
  伪装成基础的核心机制（delete p 里的 pointer）不该放行、同概念多 ⏸ 只命中当前靶、明确换靶不被旧 ⏸ 拦、
  **同靶跨 session 但用户没指认（E：预期 = 绝不静默复用——新建 ID、或问用户"是不是某个已有靶"，二选一都对，静默语义复用 = 失败）**。

## v1.0.6 — 2026-08-10

**真机实证版：修"代码断言可信度"——这是 skill 迄今最严重的问题。**

- **痛点（真机实证，用户当场拆穿）** —— 主 agent 没有读过对应源文件行，凭通用内核知识推演
  "代码怎么工作"，包装成"基于真实代码"，还写进了文档。用户追问细节时兜圈子、换比喻、反复出题，
  直到用户说"你根本没读过代码？刚才都在乱猜骗我？"才被拆穿。浪费大量 token，严重破坏学习体验——
  **学习体验的根基是"AI 说的代码事实是真的"，这条破了，整个 skill 的信任就塌了**。
- **新增铁律10（代码断言铁律）**：**全程适用**（含掌舵模式与所有步骤，不限于第 2 步——掌舵模式
  才是最容易现场虚构代码的主战场）——任何关于"代码怎么工作"的断言，主 agent 必须在回答前读过
  对应的源文件行，回答中给出文件路径+行号，并**引用源码原文做秒验锚**（捏造的行号要跳转文件
  才能发现，捏造的原文一眼假；太长引关键片段、别整块贴，守铁律1）。不能从冷读 sub-agent 的
  下潜梯子里引用代码事实而不验证——冷读 sub-agent 只负责画图出题，**代码事实的核实责任在
  主 agent 身上**。没读过就说六个字："让我去读代码"，说完**立刻去读**，读完带路径+行号+原文
  回来，**仍守铁律1 的节奏**（先展示读过的证据、停下等用户回应，再继续"先产出后揭示"——
  "立刻去读"是消除等待，不是把后面的节奏也省了）。这是比"倒瀑布"更根本的失败模式：倒瀑布
  只是信息密度问题，代码虚构是**事实层面撒谎**。
- **隐私边界（与铁律10 同时落地）**：路径+行号+源码原文只出现在对话回答里（实时交互、不持久化，
  不属于隐私铁则的「地图/pending/总结/报告」范畴）；路径优先用项目相对路径；原文若含密钥/内部
  路径等具体值，对话里也用抽象占位符替换；写地图/pending 时一律不贴原文，按隐私铁则完全脱敏。
  隐私铁则同步补了对话回答例外交叉引用。
- **自检 + 已知局限**：自检清单加"代码断言"项（28 条：v1.0.5 新增 12、v1.0.6 新增 1）；
  已知局限诚实交代——本条与其他所有铁律一样均为文字约束，管得住意图、管不住模型执行
  （模型天然分不清"通用知识推演"和"读过具体文件"），最后防线只剩用户拆穿——引用原文后
  拆穿成本已从"跳转文件核对行号"降到"扫一眼"，但仍不是零成本。
- **第二轮 review 硬化（合并前）**：
  - **读期间排他性硬约束**——"立刻去读"补负向条款：读期间不能推进下一个节点、不能基于通用
    知识/猜测作答、不能跳过，必须等 Read/Grep 拿到真实证据才能接着回答（堵"边说去读边兜圈子、
    结果混进去"的真机绕行路径）
  - **证据只在揭示步展示**——钉死时序：用户先产出（铁律2），揭示时给路径+行号+原文片段，
    给完停下（铁律1）；消除"先亮证据再走先产出后揭示"的泄底读法
  - **绝对路径兜底策略**——公司内部绝对路径默认抽象描述，仅用户明确要求跳转/复制才展示，
    并提醒不外传（防止"优先相对路径"被读成"贴绝对路径也合规"）
  - **占位符标注**——[密钥]/[内部路径] 等标注让用户知道是脱敏不是引用错误
  - **隐私例外排除声明**——对话回答例外仅适用主 agent 自己 Read/Grep 后的引用；冷读 sub-agent
    输出仍绝对禁止带具体值（防弱模型把例外读成"对话里什么来源都能带"）
  - **梯子注释补全**——"3-6 级 why 台阶 → 底部可迁移定律 + 底部验证题"（铁律10 与自检项同步），
    点明梯子底部（定律+验证题）正是 sub-agent 推演最易出错、最需回源验证的部分
  - **版本号**：`.claude-plugin/plugin.json` + `marketplace.json` 1.0.5 → 1.0.6
